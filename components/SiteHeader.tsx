...
/** The black top bar: MSTP DYNASTY in pixel caps, the week or draft status, a dithered ornament block (never under text), and the desktop nav. */
  ...
  return (
    <header className="on-ink border-b-2 border-paper bg-ink text-paper">
      <div className="mx-auto flex max-w-[1440px] items-center gap-x-4 gap-y-2 px-4 pb-3 pt-3.5 max-sm:flex-wrap md:gap-x-6 md:px-6 md:py-4">
        <Link href="/" className="type-display -my-1.5 inline-flex min-h-11 shrink-0 items-center py-1.5 text-j2 no-underline md:text-j3" aria-label="MSTP Dynasty, home">
          MSTP Dynasty
        </Link>
        ...
        <div className="order-3 flex min-w-0 basis-full items-center gap-3 sm:order-none sm:basis-auto sm:border-l-2 sm:border-paper sm:pl-4">
          ...
          <p className="type-label m-0 flex min-w-0 flex-wrap gap-x-2 gap-y-1 text-paper sm:flex-col sm:whitespace-nowrap">
            <span>{status.primary}</span>
            {status.secondary ? (
              <span className="sm:text-paper-shade">
      ...
      <nav aria-label="Main" className="hidden border-t-2 border-paper md:block">
        <div className="mx-auto flex max-w-[1440px] items-stretch pl-1 pr-4 md:pr-6 lg:pl-2">
          <NavLinks />
        </div>
      </nav>
    </header>
