...
describe.skipIf(!hasFixtures())("roastIssue", () => {
  ...
  it("stores what the writer used, and shows the next issue PREVIOUS after LORE", async () => {
    ...
    setRoastClient(client);
    const first = await roastIssue("weekly_recap", facts, ctx, { now: NOW });
    ...
    await saveIssue({ ...first, slug: "2030-10-01-weekly-recap", date: "2030-10-01", status: "sent" });
    const { client: c2, calls } = fakeClient([{ text: reply }]);
    setRoastClient(c2);
    await roastIssue("weekly_recap", facts, ctx, { now: NOW });
    ...
    await store.del(store.keys.issue(ctx.leagueId, "2030-10-01-weekly-recap"));
  });
...
describe("roastItem", () => {
  ...
  it("The Daily: a headline, a fake epic on the worst drafter, one hit per other manager (commissioner last), and a closer with the resume time", () => {
    ...
    const daily: DailyFacts = { kind: "daily", date: "2030-10-08", sinceMs: 0, trades: [], waivers: [], injuries: [], lineupAlerts: [], draftPicks: picks, hasMaterial: true };
    const mem = {
      ...EMPTY_MEMORY,
      commissioner: "Rory",
  ...
  it("issue names: The Daily, Week N Recap, never Roast", () => {
    expect(issueTitle("daily", 3)).toBe("The Daily");
    expect(issueTitle("weekly_recap", 9)).toBe("Week 9 Recap");
