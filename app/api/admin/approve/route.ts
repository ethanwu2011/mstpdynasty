 ...
 *   POST  verifies the HMAC-signed, single-use token and sends to every confirmed subscriber
...
export async function GET(req: Request): Promise<Response> {
  ...
  const who = info.subscribers === 1 ? "1 confirmed subscriber" : `${info.subscribers} confirmed subscribers`;
  return htmlPage(
    "Approve",
    [
...
export async function POST(req: Request): Promise<Response> {
  ...
  const body = res.ok
    ? `<h1>Sent</h1><p>${escapeHtml(res.issue?.title ?? "The issue")} went to ${res.recipients} ${res.recipients === 1 ? "subscriber" : "subscribers"} and is live on the site.</p>`
