...
import type { SeasonPhase, ShameBoard, ShameEntry, ShameKind } from "@/lib/types";
import { ordinal } from "../_lib/format";
import { amountText, damageText, entryHref, groupByKind, KIND_ORDER, KINDS } from "./kinds";
...
function cleanBlock(m: ShameManager, season: string): RoastBlockData {
  return {
    ...
    victim: m.name,
    stat: "Clean so far",
    text: `${m.name} has not made the wall. No bench disaster, no zero-point starter, no lopsided trade, no wasted bid. Yet.`,
    receipt: [
...
function EmptyLead({ phase, season }: { phase: SeasonPhase; season: string }) {
  const line =
    phase === "pre_draft"
      ? "The startup draft comes first. Every pick gets checked against FantasyCalc, and the worst reaches go straight up here."
      : phase === "drafting"
        ? "The draft is on. Every pick gets checked against FantasyCalc, and the worst reaches go straight up here."
  ...
  return (
    <article className="flex flex-1 flex-col gap-6 md:gap-7">
      ...
      <h3 className="type-display m-0 text-j3 md:text-j4 xl:text-j5">
        ...
        <span className="board-wipe block">Yet</span>
      </h3>
...
function RapSheets({ rows, who, placeholder, empty }: { rows: Tally[]; who: string | null; placeholder: boolean; empty: boolean }) {
  ...
  return (
    ...
    >
      ...
      <p className="m-0 mt-auto px-4 py-3 text-fine text-ink-muted md:px-6 lg:px-4 xl:px-6">
        {empty ? "Ten clean records. It will not last." : "One dot per entry. Pick a name for the rap sheet."}
      </p>
    </Panel>
  );
}
...
function RecordRow({ kind, list, rows }: { kind: ShameKind; list: ShameEntry[] | undefined; rows: Tally[] }) {
  ...
  return (
    <li className="grid grid-cols-[minmax(0,1fr)_auto] items-center gap-x-4 gap-y-2 border-b border-ink px-4 py-3.5 last:border-b-0 md:grid-cols-[minmax(12rem,1.15fr)_8.5rem_minmax(0,12rem)_minmax(0,1.5fr)] md:px-6 md:py-3">
      ...
      <div className="text-right text-d30 leading-none md:text-d40">{value}</div>
...
function LedgerGroup({ kind, list, limit, who }: { kind: ShameKind; list: ShameEntry[]; limit: number | null; who: string | null }) {
  const meta = KINDS[kind];
    ...
    <section aria-labelledby={headingId} className="border-t-2 border-ink">
      ...
      {limit && list.length > limit ? (
        <p className="m-0 border-t border-ink px-4 py-3 md:px-6">
          <Link href={shameHref({ kind, who })} className="type-label link-ink inline-flex items-center gap-2 px-0.5">
        ...
        <div className="flex flex-col gap-4 px-4 pb-5 pt-6 md:px-6 md:pt-8">
          {whoName ? (
            <p className="m-0 flex flex-wrap items-center gap-x-4 gap-y-2 text-body">
              ...
              <Link href={shameHref({ kind })} className="type-label link-ink px-0.5">
                Show everyone
              </Link>
            </p>
          ) : null}
      ...
      {count === 0 ? (
        <div className={cx("px-4 pb-6 pt-6 md:px-6", (total > 0 || whoName) && "border-t-2 border-ink")}>
          <DotMatrixFill
            label={
              whoName
                ? `Nothing on ${whoName}'s rap sheet${kind ? ` for ${KINDS[kind].title.toLowerCase()}` : ""}. Yet.`
                : kind
                  ? `No ${KINDS[kind].title.toLowerCase()} yet.`
                  : "Empty. Somebody will fix that."
            }
            rows={4}
          />
        </div>
        ...
        shownKinds.map((k) => {
          ...
          return list?.length ? <LedgerGroup key={k} kind={k} list={list} limit={kind ? null : GROUP_LIMIT} who={who} /> : null;
        })
      )}
    </Panel>
  );
...
export function ShameView({ board, managers, season, phase, kind, who }: ShameViewProps) {
  if (!board) {
    return (
      <Board>
        <Panel label="Wall of shame" labelRight={<span className="text-paper-shade">Off the air</span>}>
  ...
  return (
    <Board>
      <Panel
        label="Wall of shame"
        labelRight={whoTally ? <BarLink href="/shame">Everyone</BarLink> : <span className="text-paper-shade">{season}</span>}
        span={8}
        id="lead"
      >
        {lead ? <RoastBlock {...lead} size="hero" animate headingLevel={3} /> : <EmptyLead phase={phase} season={season} />}
      </Panel>
