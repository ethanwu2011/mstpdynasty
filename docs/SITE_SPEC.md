...
- EVERY stat surface carries a mean one-liner: each standings row, each odds row, power rankings, each matchup, each team page, each trade, each shame entry, each draft pick. Generated in batches per surface (one LLM call returns a line per row), cached in the store, refreshed when the underlying facts change (daily at most for tables, instantly for new trades/picks). Without an API key the line is simply absent (never a canned fake joke).
- Odds: playoff odds and championship odds are the headline numbers everywhere odds appear (home page in season shows them per team; /odds leads with them). During and right after the startup draft, show "if the season started today" playoff and title odds from drafted rosters (projected points), updating as picks come in, with a one-liner per team.
- Trades in hindsight: every trade shows value at the time of the trade vs value now (FantasyCalc; KeepTradeCut has no public API so do not scrape it) using the daily FantasyCalc snapshots already stored; a running "worst trade in league history" leaderboard ranked by value lost since the trade, with a small value-over-time dot chart per trade. Snapshots start now, so history accrues forward.
...
- Dev fixture league: another dynasty league of Ethan's with a full completed 2025 season: use it to test scoring, facts, models and roasts end to end. Which league it is stays out of the repo: `scripts/fetch-fixtures.ts` finds it from `FIXTURE_USER_ID` + `FIXTURE_LEAGUE_NAME` (env, or the untracked `fixtures/config.json`) and downloads fixtures into `fixtures/` (gitignored). Never publish or email anything about the dev fixture league. Env `LEAGUE_ID` overrides the league for local dev.
...
Facts engine (`lib/facts/*`), deterministic and unit tested on the dev fixture league:
...
- League scoring: fantasy points from a stat line using the league's scoring_settings (including bonus_rec_te for TEs). Test: recomputed points must equal Sleeper's own `players_points` in the dev fixture league matchups within 0.01 for every player in every week.
...
- Never commit secrets, fixtures/, .data/, .review/, or anything generated from the dev fixture league (nor its id, name or owner's user id). docs/samples/ must be gitignored.
